...
from opshin.prelude import Token, FinitePOSIXTime
...
def main(
    ...
    for u in tally_utxos:
        ...
        if not isinstance(prev_tally_datum.params.end_time, FinitePOSIXTime):
            continue
        if prev_tally_datum.params.end_time.time < int(datetime.datetime.now().timestamp() * 1000):
            continue
        if prev_tally_datum.params.staking_address != to_address(staking_address):
            continue
        if proposal_id is None or prev_tally_datum.params.proposal_id == proposal_id:
            tally_utxo = u
            break
