...
from opshin.prelude import Token, Nothing, PosInfPOSIXTime
...
def main(
    ...
    while True:
        try:
            execute_permissioned_retract_requests(
                wallet=wallet,
            )
...
def get_retract_vote_permissions_from_staking_utxo(
    ...
    filtered = [
        ...
        if (
            isinstance(vp.redeemer, vote_permission_nft.DelegatedRetractVote)
            and vp.redeemer.hash().hex()
...
def execute_permissioned_retract_requests(
    wallet: str = "creator",
):
    # Load script info
    (
    ...
    for u in staking_utxos:
        ...
        vote_permissions, vote_permission_nft_tks = (
            get_retract_vote_permissions_from_staking_utxo(
                u, vote_permission_nft_policy_id
            )
        )
    ...
    try:
        ...
        BAD_REQUESTS[staking_utxo.input.transaction_id.payload.hex()].append(
            vote_permission.redeemer.hash().hex()
        )
