...
def main(
    ...
    try:
        license_recipient = pycardano.Address.decode(license_recipient)
    except Exception:
        pass
    if isinstance(license_recipient, str):
        license_recipient = get_address(license_recipient, network=network)

    assert isinstance(
        license_recipient, pycardano.Address
    ), "Could not resolve license_recipient address"
    ...
    prev_gov_state_datum = gov_state.GovStateDatum.from_cbor(
    ...
    tally_mock_metadata = {
        "title": "Example Batcher License Tally",
        "description": ["Canonical batcher license tally"],
        "short_description": ["Canonical batcher license tally"],
        "creator_name": "MuesliSwap Dev",
        "forum_link": "https://muesliswap.com",
        "proposals": [
            {
                "title": "Nothing",
                "description": ["This is an example of a proposal that does nothing."],
            },
            {
                "title": "Release License",
                "description": [
                    "This is an example of a proposal that releases a license."
                ],
            },
        ],
    }
            ...
            metadata=Metadata(
                {
                    ...
                    TALLY_METADATA_KEY: tally_mock_metadata,
                }
            )
        )
    )
            ...
            params=tally.ProposalParams(
                ...
                proposals=[
                    ...
                    licenses.LicenseReleaseParams(
                        address=to_address(license_recipient),
                        datum=NoOutputDatum(),
                        maximum_future_validity=1000000000,
                    ),
                ],
                end_time=tally.PosInfPOSIXTime(),
    ...
    builder.ttl = context.last_block_slot + 300
